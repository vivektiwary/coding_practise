 ...
 *
 * They saw it for the first time but were already getting bored to see a bunch of sentences having
 * at most 140 characters each. The only thing they liked to play with it is,
 * closing and opening tweets.
 *
 * There are N tweets on the page and each tweet can be opened by clicking on it, to see some
 * statistics related to that tweet. Initially all the tweets are closed. Clicking on an open
 * tweet closes it and clicking on a closed tweet opens it.
 * There is also a button to close all the open tweets.
 * Given a sequence of K clicks by Jack, Evan has to guess the total number of open tweets just after
 * each click. Please help Evan in this game.
 *
 * Input:
...
int getOpenTweets(int tweets [], int size) {
    int tweetCount = 0;
...
int main(int argc, char const *argv[])
{
	...
	while(k--) {
        ...
        cout << getOpenTweets(tweets, n) << endl;
	}
	return 0;
}
